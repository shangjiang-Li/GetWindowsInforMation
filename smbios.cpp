...
//:-1: error: release/smbios.o:smbios.cpp:(.text+0x1b1b): undefined reference to `_com_util::ConvertStringToBSTR(char const*)'
//    QString QS2bstr_wql = "ROOT\\CIMV2";
// BSTR bstr_wql = SysAllocString(QS2bstr_wql.toStdWString().c_str());
//    result = pLoc->ConnectServer(bstr_wql, NULL, NULL, 0, NULL, 0, 0, &pSvc);
/////////////////QT       +=  axcontainer   /////////////////////////////////////////
    ...
    QString QS2bstr = "ROOT\\WMI";
    BSTR bstr = SysAllocString(QS2bstr.toStdWString().c_str());
    result = pLoc->ConnectServer(bstr, NULL, NULL, 0, NULL, 0, 0, &pSvcSmbios);
	if (!SUCCEEDED(result)) {
		pLoc->Release();
        CoUninitialize();
		return false;
	...
	if (SUCCEEDED(result)) {
		while (pEnumerator) {
            ...
            VariantInit(&vtProp);
                    QString QS2bstr = "SMBiosData";
                    BSTR bstr = SysAllocString(QS2bstr.toStdWString().c_str());
            result = pInstance->Get(bstr, 0, &vtProp, &pvtType, NULL);
			if (SUCCEEDED(result)) {
